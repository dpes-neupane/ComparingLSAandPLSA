...
def scrapperForSetopati(URL = 'https://setopati.com/', category = 'social', save_in = 'Data-social'):
    ...
    while next_page_link is not None and i!=20: #first 20 page only
        all_news_link_nalt = soup.select('div.row.bishesh.news-cat-list > div.items.col-md-4 >a') 
        all_news_link_alt = soup.select('div.row.bishesh.news-cat-list.alt > div.items.col-md-6 >a')
        all_news_link = all_news_link_alt + all_news_link_nalt
        ...
        for link in all_news_link:
            ...
            with open("E:\\" + save_in + "\\" + filename + ".txt", 'w', encoding='utf-8') as wp:
